...
const DashboardPage: React.FC = () => {
  ...
  const quickActions = [
    { to: '/send-emails', label: 'Send Emails', desc: 'Upload Excel and send bulk emails', icon: Upload, gradient: 'from-indigo-500 via-purple-500 to-pink-500', shadow: 'shadow-indigo-500/25' },
    ...(user?.role === 'organization' ? [
      { to: '/ai-agent', label: 'AI Agent', desc: 'Query emails with AI', icon: Bot, gradient: 'from-violet-500 via-purple-500 to-fuchsia-500', shadow: 'shadow-violet-500/25' },
      { to: '/senders', label: 'Senders', desc: `${senderCount} sender accounts`, icon: Users, gradient: 'from-emerald-500 to-teal-500', shadow: 'shadow-emerald-500/25' },
    ] : []),
  ];
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      >
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm">Here's your email campaign overview</p>
          </div>
        </div>
      </motion.div>
            ...
            >
              ...
              <div className="relative p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${action.gradient} ${action.shadow} flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}>
      ...
      >
        {stats.map((stat, i) => (
          ...
          >
            <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br ${stat.color} opacity-5 rounded-bl-[3rem]`} />
